document.addEventListener("DOMContentLoaded", function () {
  ...
  if (createEventBtn) {
    createEventBtn.addEventListener("click", () => {
      ...
      if (createEventForm) {
        ...
        document.querySelector(".modal-content p.text-2xl").textContent =
          "Create Event";
        document.querySelector('button[type="submit"]').textContent =
          "Create Event";

        // Set default time values
        document.getElementById("signinStart").value = "07:30";
        document.getElementById("signinEnd").value = "08:00";
        document.getElementById("signoutStart").value = "12:00";
        document.getElementById("signoutEnd").value = "12:30";
      }
  ...
  if (createEventForm) {
    createEventForm.addEventListener("submit", (e) => {
      ...
      const reminderOption = enableReminder
        ? getSelectedReminderOption()
        : null;
      const signinStart = document.getElementById("signinStart").value;
      const signinEnd = document.getElementById("signinEnd").value;
      const signoutStart = document.getElementById("signoutStart").value;
      const signoutEnd = document.getElementById("signoutEnd").value;
      ...
      if (signinStart && signinEnd && signinStart >= signinEnd) {
        showErrorAlert(
          "Invalid Time",
          "Sign-in start time must be before sign-in end time"
        );
        return;
      }

      if (signoutStart && signoutEnd && signoutStart >= signoutEnd) {
        showErrorAlert(
          "Invalid Time",
          "Sign-out start time must be before sign-out end time"
        );
        return;
      }
      ...
      const eventData = {
        ...
        signout_start: signoutStart,
        signout_end: signoutEnd,
      };
  ...
  function createEvent(eventData) {
    ...
    fetch(API_URL, {
      ...
      .catch((error) => {
        ...
        showErrorAlert(
          "Error",
          "Failed to create event. Please try again. Error: " + error.message
        );
      })
      .finally(() => {
        // Reset button state
    ...
    fetch(API_URL, {
      ...
      .then((data) => {
        if (data.success) {
          ...
          // Show success message
          showSuccessAlert("Success!", "Event updated successfully!").then(
            () => {
      ...
      if (result.isConfirmed) {
        ...
        fetch(API_URL, {
          ...
          .then((data) => {
            if (data.success) {
              // Show success message
              showSuccessAlert("Success!", "Event approved successfully!").then(
                () => {
        ...
        fetch(API_URL, {
          ...
          .then((data) => {
            if (data.success) {
              ...
              // Show success message
              showSuccessAlert("Success!", "Event deleted successfully!").then(
                () => {
    ...
    fetch(`${API_URL}?id=${eventId}`)
      ...
      .then((data) => {
        if (data.success && data.data.length > 0) {
          ...
          const signinTime =
            event.signin_start && event.signin_end
              ? `${event.signin_start} - ${event.signin_end}`
              : "Not set";
          const signoutTime =
            event.signout_start && event.signout_end
              ? `${event.signout_start} - ${event.signout_end}`
              : "Not set";
          ...
          if (detailsContainer) {
            detailsContainer.innerHTML = `
                        <div class="mb-4">
                            ...
                            <p class="text-gray-400">${formattedDate} at ${
              event.venue
            }</p>
                        </div>
                        <div class="mb-4">
                        ...
                        <div class="mb-4">
                            <div class="flex justify-between mb-2">
                                <span class="text-sm font-bold">Sign-in Time:</span>
                                <span class="text-sm">${signinTime}</span>
                            </div>
                            <div class="flex justify-between mb-2">
                                <span class="text-sm font-bold">Sign-out Time:</span>
                                <span class="text-sm">${signoutTime}</span>
                            </div>
                        </div>
              ...
              event.status
                            ...
                            <div>
                                ...
                                <span class="text-sm">${
                                  event.reminder_enabled ? "Active" : "Not set"
                                }</span>
                            </div>
                        </div>
                    `;
          ...
          const container = document.querySelector(
            "#viewEventModal .modal-container"
          );
          ...
          if (bgInput) {
            ...
            if (!removeBtn) {
              ...
              removeBtn.className =
                "inline-flex items-center px-3 py-2 bg-red-500 text-white rounded hover:bg-red-600 ml-2 text-sm";
      ...
      .then((data) => {
        if (data.success && data.data.length > 0) {
          ...
          if (event.reminder_time) {
            const reminderSelect = document.querySelector(
          ...
          // Set sign-in/sign-out time fields
          document.getElementById("signinStart").value =
            event.signin_start || "";
          document.getElementById("signinEnd").value = event.signin_end || "";
          document.getElementById("signoutStart").value =
            event.signout_start || "";
          document.getElementById("signoutEnd").value = event.signout_end || "";
  ...
  function filterEventsByStatus(status) {
    ...
    eventRows.forEach((row) => {
      ...
      if (statusCell) {
        row.style.display =
          status === "all" || statusCell.classList.contains(status)
            ? ""
            : "none";
      }
    });
