...
// Determine size: enforce square and bounds per goQR docs (10..1000 px for raster)
$requestedSize = isset($_GET['size']) ? preg_replace('/[^0-9x]/', '', $_GET['size']) : '64x64';
if (preg_match('/^(\d+)x(\d+)$/', $requestedSize, $m)) {
    $w = max(10, min(1000, (int)$m[1]));
    $h = max(10, min(1000, (int)$m[2]));
    $side = min($w, $h); // keep square as required by API examples/best practice
    $size = "{$side}x{$side}";
} else {
    $size = '64x64';
}
...
$query = [
    ...
    'margin' => $margin,
    'format' => $format,
    // Charset hints to avoid special character issues, per docs
...
if ($color) { $nameParts[] = "c-{$color}"; }
if ($bgcolor) { $nameParts[] = "bg-{$bgcolor}"; }
$fileName = implode('__', $nameParts) . '.png';
